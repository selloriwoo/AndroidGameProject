...
import com.JaeRak.mygallag.sprites.AlienSprite;
import com.JaeRak.mygallag.sprites.Sprite;
import com.JaeRak.mygallag.sprites.StarshipSprite;

import java.util.ArrayList;
import java.util.Random;

public class SpaceInvadersView extends SurfaceView implements Runnable, SurfaceHolder.Callback {
    //surfaceView 는 스레드를 이용해 강제로 화면에 그려주므로 View 보다 빠르다. 애니메이션, 영상 처리 이용
    ...
    private volatile boolean running; //휘발성 불 함수
    private Canvas canvas;
    int mapBitmapY = 0;

    public SpaceInvadersView(Context context, int characterId, int x, int y) { //MainActivity, Intent(침략자)
        super(context);
